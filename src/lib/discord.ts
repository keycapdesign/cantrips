...
export async function sendDealNotification(game: Game, deal: Deal) {
  const webhookUrl = process.env.DISCORD_DEALS_WEBHOOK;
  ...
  const embed = {
    ...
    fields: [
      deal.flag === "H" || deal.flag === "N"
        ...
        : null,
      deal.expires_at
        ? { name: "Expires", value: deal.expires_at, inline: true }
        : null,
...
export async function sendDealBatch(games: Game[], deals: Deal[]) {
  const webhookUrl = process.env.DISCORD_DEALS_WEBHOOK;
  if (!webhookUrl) return;
