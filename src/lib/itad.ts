...
export interface ITADSearchResult {
  ...
  type: string | null;
  mature: boolean;
  assets: { boxart: string; banner145: string; banner300: string; banner400: string; banner600: string } | null;
}
...
export class ITADClient {
  ...
  private params(extra?: Record<string, string>): URLSearchParams {
    const p = new URLSearchParams({ key: this.apiKey, ...extra });
    return p;
  }
  ...
  async getHistory(
    ...
    const url = `${ITAD_BASE}/games/history/v2?${params}`;
    const res = await fetch(url);
    if (!res.ok) {
      const body = await res.text();
      console.error(`ITAD history failed: ${res.status}`, url, body);
      throw new Error(`ITAD history failed: ${res.status}`);
    }
    return res.json();
  }
...
export const itad = new ITADClient(process.env.ITAD_API_KEY || "");
